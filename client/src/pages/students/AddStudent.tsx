...
import { UserPlus, ArrowLeft, Save, CheckCircle2, Lock, AlertCircle, ChevronDown } from 'lucide-react';
...
const studentSchema = z.object({
  ...
  studentId: z.string().min(3, 'Student ID / Roll No is required'),
...
export const AddStudent = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const res = await api.get('/students/check-email', { params: { email: typedEmail } });
        if (res.data?.data?.exists) {
          const s = res.data.data.student;
          setEmailCheck({
            checking: false,
            exists: true,
            message: `⚠️ Student email is already registered to ${s?.name || 'an existing student'} (Roll No: ${s?.studentId || 'N/A'})!`,
          });
        } else {
          setEmailCheck({
            checking: false,
  ...
  // Fetch unchangeable sequential Student ID when a department is selected
  useEffect(() => {
    if (!selectedDeptId) {
      setValue('studentId', 'Select department first...');
      return;
    }

    const fetchNextId = async () => {
      try {
        const res = await api.get(`/students/next-id`, { params: { departmentId: selectedDeptId } });
        if (res.data?.data?.studentId) {
          setValue('studentId', res.data.data.studentId);
        }
      } catch {
        ...
        if (code === 'CSE') code = 'CS';
        if (code === 'ECE') code = 'EC';
        setValue('studentId', `2026${code}005`);
      }
    };
  ...
  const mutation = useMutation({
    mutationFn: (data: StudentFormData) => studentService.create({ ...data, lastName: data.lastName || '' }),
    onSuccess: () => {
      ...
      <div className="rounded-3xl border border-amber-200/80 bg-white p-6 md:p-8 shadow-sm dark:border-stone-700/50 dark:bg-[#0d0d10] space-y-6">
        ...
        {successMsg && (
          <div className="rounded-2xl bg-emerald-50 p-4 border border-emerald-200 text-emerald-800 dark:bg-emerald-950/60 dark:border-emerald-800 text-xs font-bold flex items-center gap-2">
            ...
            <span>Student successfully registered! Default login password is <code className="bg-emerald-200 dark:bg-emerald-900 px-1.5 py-0.5 rounded font-mono text-emerald-950 dark:text-emerald-100 font-bold">student123</code>. Redirecting...</span>
          </div>
        )}
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="block text-xs font-extrabold uppercase tracking-wider text-stone-700 dark:text-stone-300">
                  Student ID / Roll No <span className="text-rose-500 font-bold">*</span>
                </label>
              ...
              />
              {errors.studentId && <p className="mt-1 text-[11px] font-bold text-rose-500">{errors.studentId.message}</p>}
            </div>
            ...
            <div>
              ...
              {errors.gender && <p className="mt-1 text-[11px] font-bold text-rose-500">{errors.gender.message}</p>}
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-2xl border border-amber-200 bg-amber-50/40 px-4 py-2.5 text-sm text-stone-900 focus:border-amber-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-amber-500/20 dark:border-stone-600 dark:bg-[#111113] dark:text-white dark:placeholder:text-stone-500 dark:focus:border-emerald-500 dark:focus:ring-emerald-500/30 dark:focus:bg-[#111113]"
              />
              {errors.dateOfBirth && <p className="mt-1 text-[11px] font-bold text-rose-500">{errors.dateOfBirth.message}</p>}
            </div>

            {/* Enrollment Date (Auto-filled to today) */}
            <div>
              <label className="block text-xs font-extrabold uppercase tracking-wider text-stone-700 dark:text-stone-200 mb-1.5">
                Enrollment Date <span className="text-rose-500 font-bold">*</span>
              </label>
              <input
                ...
                className="w-full rounded-2xl border border-amber-200 bg-amber-50/40 px-4 py-2.5 text-sm text-stone-900 focus:border-amber-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-amber-500/20 dark:border-stone-600 dark:bg-[#111113] dark:text-white dark:placeholder:text-stone-500 dark:focus:border-emerald-500 dark:focus:ring-emerald-500/30 dark:focus:bg-[#111113]"
              />
              {errors.enrollmentDate && <p className="mt-1 text-[11px] font-bold text-rose-500">{errors.enrollmentDate.message}</p>}
            </div>

            {/* Status */}
            <div>
              <label className="block text-xs font-extrabold uppercase tracking-wider text-stone-700 dark:text-stone-200 mb-1.5">
                Student Status <span className="text-rose-500 font-bold">*</span>
              </label>
              <div className="relative">
                <select
                  {...register('status')}
          ...
          <div>
            ...
            <textarea
              {...register('address')}
              rows={2}
              placeholder="Enter campus hall or residential address"
              className="w-full rounded-2xl border border-amber-200 bg-amber-50/40 px-4 py-2.5 text-sm text-stone-900 focus:border-amber-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-amber-500/20 dark:border-stone-600 dark:bg-[#111113] dark:text-white dark:placeholder:text-stone-500 dark:focus:border-emerald-500 dark:focus:ring-emerald-500/30 dark:focus:bg-[#111113]"
            />
            {errors.address && <p className="mt-1 text-[11px] font-bold text-rose-500">{errors.address.message}</p>}
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-amber-100 dark:border-stone-800">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-amber-500 via-orange-500 to-rose-500 px-6 py-2.5 text-xs font-black text-white shadow-lg shadow-orange-500/25 hover:shadow-orange-500/40 disabled:opacity-50 transition-all cursor-pointer font-['Outfit']"
            >
              <Save className="h-4 w-4" />
              {mutation.isPending ? 'Saving Record...' : 'Save Student'}
            </button>
          </div>
        </form>
      </div>
